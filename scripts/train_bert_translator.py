...
for (step, (input_ids, target_ids_)) in tqdm(enumerate(train_dataset)):
  ...
  if eval_frequency == 0:
    predicted = tokenizer.decode([i for i in tf.squeeze(tf.argmax(refine_predictions,axis=-1)) if i not in [CLS_ID, SEP_ID, 0]])
    target = tokenizer.decode([i for i in tf.squeeze(target_x) if i not in [CLS_ID, SEP_ID, 0]])
